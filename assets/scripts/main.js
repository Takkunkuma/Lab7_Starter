...
async function getRecipes() {
  ...
  let recipeItem = window.localStorage.getItem('recipe');
  if(recipeItem){
  ...
  return new Promise(async function(resolve, reject){
    ...
    for(let index in RECIPE_URLS){
      ...
      try {
        ...
        let response = await fetch(RECIPE_URLS[index]);
        //console.log("Got through A6 with: ", response)
        ...
        //            resolve() method.
        if(RECIPE_URLS.length == recipeHolder.length){
